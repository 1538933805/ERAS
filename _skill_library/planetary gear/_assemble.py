...
#@markdown
"设置被抓取的位姿"
pos_c = [325.25e-3, -653.7e-3, 42e-3, 0, -3.1415926, -1.5707963]
# env.rob.moveIK(pos_c[0:3], pos_c[3:6], wait=True)
pos_c_up = pos_c.copy()
pos_c_up[2] += 100e-3
"设置被装配的位姿"
pos_a = [-6.5e-3, -617.65e-3, 91e-3, 0, -3.1415, 0]
...
env.rob.closeRG2()
#往下抵住
env.rob.moveIK_changePosOrt(d_pos=[0e-3,0e-3,-7e-3], d_ort=[0,0,0], wait=True)
# time.sleep(2)
# 被抓取位置上方
env.rob.moveIK(pos_c_up[0:3], pos_c_up[3:6], wait=True, acc=0.025, vel=0.05)
...
while True:
    ...
    # else:
    #     env.reset(isAddError=False, is_drag_mode=False)
    env.reset(isAddError=True, is_drag_mode=False, error_scale_pos=1, error_scale_ort=1)
    ...
    for ps in range(pred_steps):
        ...
        for i in range(len(action)):
            ...
            if env.done or env.assembleDepth>0.01:
                break
